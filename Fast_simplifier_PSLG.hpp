...
public:
    ...
    explicit Fast_simplifier_PSLG(const std::string& input_folder_name, bool gen_test = false,
...
private:
    ...
    unsigned long long get_vertices_left() const;


    CDT ct; // triangulation
    std::vector<CDT::Vertex_handle> gid_to_vh; // size = init_global_vertex_count, vh->info() == gid
};
